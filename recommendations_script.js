...
let _currentRankings = [];
let _currentMainResult = null;
...
function initializeCharts(rankingData = null, mainResult = null) {
  ...
  if (recycleCtx) {
    ...
    if (rankingData && rankingData.length >= 5) {
      ...
      data = rankingData.slice(0, 5).map((m, idx) => 95 - (idx * 6));
    }
...
async function getRecommendation() {
  try {
    ...
    // Store main result for export
    _currentMainResult = data;
    ...
    try {
      const envResp = await fetch('/environment-score', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          strength: Number(strength),
...
async function generateAndDisplayRanking(mainResult, category, weight, strength, bio, recycle) {
  try {
    ...
    let rankResp = await tryFetch(window.location.origin);
    ...
    if (rankResp && rankResp.ok) {
      ...
      const resp = await fetch('/materials', { method: 'GET' });
      if (resp.ok) {
        const materialsMap = await resp.json();
...
function renderRankingTable(rankings) {
  // Store rankings for export
  _currentRankings = rankings || [];
...
function displayMetrics(result, weight) {
  ...
  metricsContainer.innerHTML = `
    <h4 style="color: #d0e7d1; text-align: center; margin-bottom: 30px;">Comparison Metrics</h4>
...
async function exportExcel() {
  try {
    ...
    // Prepare ranking data (top 5 recommendations)
    const topRecommendations = _currentRankings.slice(0, 5).map((item, idx) => ({
      rank: idx + 1,
    ...
    downloadBlob(blob, 'Material_Recommendations.xlsx');
